...
public class TestInformationKit
{
    ...
    public synchronized static void endTestSuite()
    {
        final DecimalFormat fmt = new DecimalFormat("#0.00");
        DecimalFormat dfPerCent = new DecimalFormat("#0.0#%");
        final double perErrors = suiteTotalTestCases == 0 ? 0 : (double) suiteTestErrorCount / (double) suiteTotalTestCases;

        final float suiteTimeSec = (System.currentTimeMillis() - suiteStartTime) / 1000f;
        final String suitTimeSecMsg = (suiteTimeSec == 1 ? "1 second" : fmt.format(suiteTimeSec) + " seconds");
        final float suiteTimeMins = suiteTimeSec / 60;
        final String suitTimeMinsMsg = (suiteTimeSec == 1 ? "1 minute" : fmt.format(suiteTimeMins) + " minutes");
        ...
        {
            final float runtimeSecF = tci.runtimeMS / 1000f;
            final String runtimeMsg = (runtimeSecF == 1 ? "1 second" : fmt.format(runtimeSecF) + " seconds (" + tci.runtimeMS + "ms)");

            final double msPerRequest = tci.httpRequestCount == 0 ? 0 : (double) tci.httpTimeMS / (double) tci.httpRequestCount;

            sb.append("\t").append(tci.testName).append(" ").append(tci.testPassed ? "PASSED" : "FAILED").append("\n");
            sb.append("\t\t").append(padR40("Run Time")).append(" : ").append(runtimeMsg).append("\n");
            sb.append("\t\t").append(padR40("HTTP Count")).append(" : ").append(tci.httpRequestCount).append("\n");
            sb.append("\t\t").append(padR40("HTTP Time")).append(" : ").append(tci.httpTimeMS).append("ms\n");
            sb.append("\t\t").append(padR40("HTTP Ave")).append(" : ").append(fmt.format(msPerRequest / 1000d)).append(" ms/request ");
            ...
            final List<TestCaseCounter> counters = getCountersForTest(tci.testName);
            for (TestCaseCounter counter : counters)
            {
                sb.append("\t\t").append(padR40(counter.getName())).append(" : ").append(counter.getValue()).append("\n");
            }
            sb.append("\n");
        }
        ...
        sbSummary.append("\n")
                ...
                .append("\t").append(padR40("Run Time")).append(" : ").append(suitTimeMinsMsg).append(" (").append(suitTimeSecMsg).append(")\n")
                .append("\t").append(padR40("Tests Run")).append(" : ").append(suiteTotalTestCases).append("\n")
                .append("\t").append(padR40("Errors")).append(" : ").append(suiteTestErrorCount).append(" (").append(dfPerCent.format(perErrors)).append(")\n")
                .append("\n");
        sb.append(sbSummary);
        sb.append(new MsgOfD());

        FuncTestLogger log = new FuncTestLoggerImpl(0);
        log.log(sb);
    }
    ...
    private static StringBuilder padR40(final String s)
    {
