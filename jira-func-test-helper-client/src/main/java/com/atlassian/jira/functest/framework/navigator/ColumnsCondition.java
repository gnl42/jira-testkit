...
public class ColumnsCondition implements SearchResultsCondition
{
    ...
    public void assertCondition(final WebTester tester)
    {
        ...
        {
            final int columnMax = table.getColumnCount();
            int columnPos = 0;
            ...
            if (columnPos < columnMax)
            {
                fail ("Exepcted columns '" + columns + "' to be visible: Found extra column '" + StringUtils.trimToNull(table.getCellAsText(0, columnPos)) + "'.");
            }
        }
    }
}
