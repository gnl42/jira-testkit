...
public class CollectionEnclosedIterable<T> implements EnclosedIterable<T>
{
    ...
    CollectionEnclosedIterable(@com.atlassian.jira.tests.util.NotNull final Collection<? extends T> collection)
    {
        this.collection = Assertions.notNull("collection", collection);
    }

    public void foreach(final com.atlassian.jira.tests.util.Consumer<T> sink)
    {
