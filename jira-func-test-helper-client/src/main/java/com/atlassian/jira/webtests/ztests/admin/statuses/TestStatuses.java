...
public class TestStatuses extends FuncTestCase
{
    public void testWorkflowLinksUrlEncoded()
    {
        ...
        navigation.gotoAdminSection("statuses");
        tester.assertTextNotPresent("workflowName=<xxx>Crazy workflow</xxx>");
        tester.assertTextPresent("workflowName=%3Cxxx%3ECrazy+workflow%3C%2Fxxx%3E");
    }
    ...
    public void testDeleteActiveStatusViaUrlFails() throws Exception
    {
        ...
        tester.assertTextNotPresent("The status with ID 1 is currently associated with a workflow(s) and cannot be deleted.");
        tester.submit("Delete");

        // Assert user gets the error message
        tester.assertTextPresent("Delete Status: Open");
        tester.assertTextPresent("Confirm that you want to delete this status ...");
        tester.assertTextPresent("The status with ID 1 is currently associated with a workflow(s) and cannot be deleted.");
    }
    ...
    {
        assertions.getJiraFormAssertions().assertFieldErrMsg("A status with that name already exists.");
    }
}
