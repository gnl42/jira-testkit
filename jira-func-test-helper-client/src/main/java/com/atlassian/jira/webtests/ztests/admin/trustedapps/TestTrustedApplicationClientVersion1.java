...
public class TestTrustedApplicationClientVersion1 extends JIRAWebTest
{
    ...
    public void _testClientLoggedInFred() throws Exception
    {
        ...
        final WebClient webClient = testContext.getWebClient();
        final WebResponse response = webClient.sendRequest(request);
        assertResponseContainsNoErrors(token, response);
    ...
    private void assertResponseContainsNoErrors(Token token, WebResponse response)
    {
        String msg = response.getHeaderField(TrustedAppHeader.ERROR);
        assertNotNull(msg, response);
        assertNotNull(msg, response.getHeaderField(TrustedAppHeader.STATUS));
        assertEquals(msg, Status.OK, response.getHeaderField(TrustedAppHeader.STATUS));
        assertNull(msg, response.getHeaderField(TrustedAppHeader.ERROR));
    }
