...
public class TestBrowseZipAttachmentEntries extends FuncTestCase
{
    ...
    private void copyAttachmentFilesToJiraHome()
    {
        ...
        final File testAttachmentsPath = new File(environmentData.getXMLDataLocation(), "TestBrowseZipAttachmentEntries/attachments");

        try
        {
            FileUtils.copyDirectory(testAttachmentsPath, jiraAttachmentsPath);
            // Bad file names don't live well in svn.  Need to rename it.
            FileUtils.moveFile(new File(administration.getCurrentAttachmentPath() +"/HSP/HSP-4/10040_tempName.zip"), new File(administration.getCurrentAttachmentPath() +"/HSP/HSP-4/10040_{[('BadFileName.zip"));
        }
        catch (IOException e)
        {
    ...
    private void removeAttachmentFilesFromJiraHome()
    {
        final File jiraAttachmentsPath = new File(administration.getCurrentAttachmentPath());
        try
        {
