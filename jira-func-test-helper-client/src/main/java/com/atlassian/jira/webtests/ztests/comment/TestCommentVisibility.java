...
public class TestCommentVisibility extends FuncTestCase
{
            ...
            final String[] expectedPresent, final String[] expectedAbsent)
    {
        ...
        tester.getDialog().setWorkingForm("issue-filter");

        if (projects == null || projects.length == 0)
        {
            tester.selectOption("pid", "All projects");
        }
        else
        {
            for (String project : projects)
            {
                final String value = tester.getDialog().getValueForOption("pid", project);
                tester.checkCheckbox("pid", value);
            }
        }

        tester.checkCheckbox("body", "true");
        tester.setFormElement("query", commentBody);
        tester.submit("show");
