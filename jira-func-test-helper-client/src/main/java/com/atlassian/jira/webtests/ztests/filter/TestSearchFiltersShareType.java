...
public class TestSearchFiltersShareType extends FuncTestCase
{
    ...
    private static final FilterItem ADMIN_GROUP_FILTER = new FilterItem(10080L, "developer", "", "developer the great and wise (developer)", 0L,
    ...
    private static final FilterItem WIFE_GROUP_FILTER = new FilterItem(10080L, "developer", "", "developer the great and wise (developer)", 0L,
    ...
    public void testFindUsingAnonymous()
    {
        ...
        String query = tester.getDialog().getResponse().getURL().getQuery();

        checkSearchResults(expectedAdminItems);

        navigation.logout();

        tester.gotoPage("secure/ManageFilters.jspa?" + query);
    ...
    public void testFindMonkeyProjectWithRoleNaggersAsWifeAndConditions(FilterNavigation filterNavigation)
    {
        ...
        executeProjectSearch(MONKEY_PROJECT_ID, NAGGERS_ROLE_ID, null, "wife", filterNavigation);
    ...
    private void addFilters(final List <FilterItem> items, final String name, final char startChar, final char endChar,
            final long initialId, final String user, final List shares)
    {
        long currentId = initialId;

        for (char currentChar = startChar; currentChar <= endChar; currentChar++)
        {
            items.add(new FilterItem(currentId, name + currentChar, "", user, 0L, shares,
                    Boolean.FALSE, 0L, Collections.EMPTY_LIST, 0L));
            currentId++;
        }
    }
            ...
            final String text, final String userName, FilterNavigation filterNavigation)
    {
        ...
        fillInOtherParameters(text, userName);
    ...
    private void fillInOtherParameters(final String text, final String userName)
    {
