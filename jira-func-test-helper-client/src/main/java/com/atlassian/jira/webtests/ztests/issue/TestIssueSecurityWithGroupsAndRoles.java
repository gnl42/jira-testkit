...
public class TestIssueSecurityWithGroupsAndRoles extends JIRAWebTest
{
    ...
    /**
     * Check that newly created projects have default roles setup by checking admin can see the security level
     * and users can only see None
     */
    ...
    public void testRenamingSecuritySchemeFlushesCache()
    {
        ...
        submit("Update");
        assertLinkWithTextExists(SECURITY_SCHEME_NAME + "edited"); //check we successfullt renamed the scheme
        assertIssueSecurityLevelOptions(EXPECTED_SECURITY_LEVEL_RED);
    }

    /**
     * Check that if the associated security scheme of a project is unassociated, that the security levels
     * are no longer visible visible
     */
