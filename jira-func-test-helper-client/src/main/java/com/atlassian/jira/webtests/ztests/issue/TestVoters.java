...
public class TestVoters extends FuncTestCase
{
    ...
    {
        gotoIssue(ISSUE_WITH_VOTE);
        assertCantVote(ISSUE_WITH_VOTE, MSG_CANT_VOTE_REPORTER, MSG_CANT_VOTE_REPORTER_ERROR, 1);
    }
    ...
    public void testUserVoting()
    {
        ...
        //Lets try to vote directly and make sure that it does not work.
        voteDirectlyForIssue(ISSUE_WITH_VOTE);
        tester.assertTextPresent(MSG_CANT_ADD_VOTE);
        gotoIssue(ISSUE_WITH_VOTE);
        assertUserVoted();
        assertEquals(1, getVoteCount());
        ...
        //Lets try to unvote directly and make sure that it does not work.
        unVoteDirectlyForIssue(ISSUE_WITH_VOTE);
        tester.assertTextPresent(MSG_CANT_REMOVE_VOTE);
        gotoIssue(ISSUE_WITH_VOTE);
        assertUserNotVoted();
        assertEquals(0, getVoteCount());

        //Lets vote again to ensure that it works.
        tester.clickLink(ID_VOTE_LINK);
        assertUserVoted();
        assertEquals(1, getVoteCount());
        navigation.login(ADMIN_USERNAME);
    ...
    private void assertCantVote(final Issue issue, final String cantVoteTitle, final String cantVoteError, final int votes)
    {
        gotoIssue(issue);
        //Should not be able to vote.
        tester.assertLinkNotPresent(ID_VOTE_LINK);
        tester.assertLinkNotPresent(ID_VOTE_ACTION);
        //Should see this text when we can't vote.
