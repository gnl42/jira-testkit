...
public class TestDeleteHiddenFieldOnMove extends FuncTestCase
{
    ...
    public void testSingleMove()
    {
        ...
        tester.selectOption("pid", "Rattus");
        // Go to step 3
        tester.submit("Next >>");
        ...
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"", "Original Value (before move)", "New Value (after move)"});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Project", "Bovine", "Rattus"});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Type", "Bug", "Bug"});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Fix Version/s", "1.1", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Stuff", "stuff value 1", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Milk", "milk value 1", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Priority", "Major", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Resolution", "Fixed", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Environment", "the environment", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Affects Version/s", "1.0", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Description", "Hello", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Milk2", "milk value 2", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Assignee", "Daisy Doolittle", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] { "Reporter", ADMIN_USERNAME, "" });
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Time Tracking", "1w 3d", ""});
        assertions.getTableAssertions().assertTableContainsRowOnce(move_confirm_table, new String[] {"Due Date", "27/Oct/09", ""});
    ...
    public void testBulkMove()
    {
        ...
        // Click Link 'all 5 issue(s)' (id='bulkedit_all').
