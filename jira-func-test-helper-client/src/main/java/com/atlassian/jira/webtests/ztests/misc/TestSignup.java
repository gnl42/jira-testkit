...
public class TestSignup extends FuncTestCase
{
    ...
    private void assertMessageOnLoginPage(boolean publicModeEnabled)
    {
        ...
        if (publicModeEnabled)
        {
            ...
            assertions.getTextAssertions().assertTextSequence(signUpHintLocator(),
                    new String[] { "Not a member?", "Sign up", "for an account." });
            assertions.getTextAssertions().assertTextNotPresent(signUpHintLocator(), "to request an account.");
            assertions.link().assertLinkByIdHasExactText("signup", "Sign up");
        }
        else
        {
            // Not a member? Signup for an account.
            assertions.text().assertTextNotPresent(signUpHintLocator(), "for an account");
            // Not a member? Contact an Administrator to request an account.
            assertions.text().assertTextSequence(signUpHintLocator(),
    ...
    public void testMustBeLoggedOutToSignUp()
    {
        ...
        assertions.getTextAssertions().assertTextSequence(locator.page(),
