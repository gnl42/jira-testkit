...
public class TestCustomFieldDoesItFitMultiple extends AbstractJqlFuncTest
{
    ...
    public void testRadioButton() throws Exception
    {
        ...
        assertFitsFilterForm("project = THIRTEEN AND RB = rad3");
        assertFitsFilterForm("type = Bug AND RB = rad3", cfParam3, typeParam);
        assertFitsFilterForm("type = Bug AND RB = rad2");
    ...
    public void testSelectList() throws Exception
    {
        ...
        final String fieldConfigSchemeId = "10024";
        final String proj1 = "10024";
        final String proj2 = "10025";

        final IssueNavigatorAssertions.FilterFormParam projParam = createFilterFormParam("pid", proj1);
        final IssueNavigatorAssertions.FilterFormParam typeParam = createFilterFormParam("type", TYPE_BUG);
        final IssueNavigatorAssertions.FilterFormParam cfParam1a = createFilterFormParam("customfield_10014", "10008");
        final IssueNavigatorAssertions.FilterFormParam cfParam1b = createFilterFormParam("customfield_10014", "10070");
        final IssueNavigatorAssertions.FilterFormParam cfParam2 = createFilterFormParam("customfield_10014", "10009");
        final IssueNavigatorAssertions.FilterFormParam cfParam3 = createFilterFormParam("customfield_10014", "10037");

        // one context for Project FIFTEEN one for Issue Type BUG
        // FIFTEEN: select1, select2
        // BUG: select1, select3
        assertFitsFilterForm("project = FIFTEEN AND SL = select2", cfParam2, projParam);
        assertFitsFilterForm("project = FIFTEEN AND SL = select3");
        assertFitsFilterForm("type = Bug AND SL = select3", cfParam3, typeParam);
        assertFitsFilterForm("type = Bug AND SL = select2");

        assertFitsFilterForm("project = FIFTEEN AND SL = select1", projParam, cfParam1a);
        assertTooComplex("project = FIFTEEN AND SL IN (select1, select2)");
        assertTooComplex("project = FIFTEEN AND SL IN (select1, select3)");
        assertFitsFilterForm("type = Bug AND SL = select1", typeParam, cfParam1b);
        assertTooComplex("type = Bug AND SL IN (select1, select3)");
        assertTooComplex("type = Bug AND SL IN (select1, select2)");
        assertFitsFilterForm("project = FIFTEEN AND type = Bug AND SL = select1", projParam, typeParam, cfParam1a);
        ...
        // remove other context before continuing
        administration.customFields().removeConfigurationSchemeContextById(customFieldId, "10044");

        final List<CustomFieldContext> contexts = generateContexts(proj1, proj2, TYPE_BUG, TYPE_TASK);
        final List<Query> queries = generateQueries("SL = select2", proj1, proj2, TYPE_BUG, TYPE_TASK);
    ...
    public void testSingleVersionPicker() throws Exception
    {
        ...
        assertFitsFilterForm("project = SIXTEEN AND SVP = 10010", projParam, cfParam);
        ...
        assertFitsFilterForm("project = SIXTEEN AND SVP in (10010, 10011)", projParam, createFilterFormParam("customfield_10015", "10010", "10011"));
    ...
    public void testVersionPicker() throws Exception
    {
        ...
        assertFitsFilterForm("project = TWENTY AND VP = 10013", projParam, cfParam);
        ...
        assertFitsFilterForm("project = TWENTY AND VP in (10013, 10014)", projParam, createFilterFormParam("customfield_10019", "10013", "10014"));
        ...
        for (final List<String> issueType : issueTypes)
        {
            for (final List<String> project : projects)
            {
                if (issueType != null || project != null)
                {
                    ...
                    {
                        @Override
                        public boolean shouldFit(final CustomFieldContext context)
                        {
                            return project != null && project.size() < 2 && super.shouldFit(context);
                        }
