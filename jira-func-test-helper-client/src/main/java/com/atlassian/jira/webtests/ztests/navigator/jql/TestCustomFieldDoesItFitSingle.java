...
public class TestCustomFieldDoesItFitSingle extends AbstractJqlFuncTest
{
    ...
    public void testMultiUserPicker() throws Exception
    {
        ...
        Map<String, Result> matrix = new LinkedHashMap<String, Result>();
        matrix.put("= fred", new Result(createFilterFormParam("customfield_10009", FRED_USERNAME)));
        ...
        // switch to user & group searcher
        administration.customFields().setCustomFieldSearcher(customFieldId, "com.atlassian.jira.plugin.system.customfieldtypes:userpickergroupsearcher");
...
//        final String customFieldId = "10015";
    ...
    public void testUserPicker() throws Exception
    {
        ...
        Map<String, Result> matrix = new LinkedHashMap<String, Result>();
        matrix.put("= fred", new Result(createFilterFormParam("customfield_10018", FRED_USERNAME)));
        matrix.put("!= fred", Result.tooComplex());
        addComplexEmpty(matrix);
        matrix.put("in (fred, admin)", Result.tooComplex());
        matrix.put("not in (fred, admin)", Result.tooComplex());
...
//        final String customFieldId = "10019";
    ...
    private void _testFieldDoesntExistWhenNoSearcher(final String customFieldId, final String fieldName, final Map<String, Result> matrix)
    {
        // remove searcher id
