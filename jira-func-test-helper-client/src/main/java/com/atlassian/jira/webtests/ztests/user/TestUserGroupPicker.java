...
public class TestUserGroupPicker extends JIRAWebTest
{
    ...
    {
        logSection("Searching with Group searcher");

        displayAllIssues();
        assertTextPresent(THREE_ISSUES);
        selectOption(USER_FIELD_ID + "Select", "Current User");
        submit("show");
        assertTextPresent(TWO_ISSUES);


        selectOption(USER_FIELD_ID + "Select", "Specify User");
        setFormElement(USER_FIELD_ID, ADMIN_USERNAME);
        submit("show");
        assertTextPresent(TWO_ISSUES);
        assertTextPresent(SUMMARY_BOTH);
        assertTextPresent(SUMMARY_ADMIN);
        assertTextNotPresent(SUMMARY_FRED);


        setFormElement(USER_FIELD_ID, FRED_USERNAME);
        submit("show");
        assertTextPresent(SUMMARY_FRED);
        assertTextPresent(TWO_ISSUES);
        assertTextPresent(SUMMARY_BOTH);


        selectOption(USER_FIELD_ID + "Select", "Specify Group");
        setFormElement(USER_FIELD_ID, "jira-administrators");
        submit("show");
        assertTextPresent(SUMMARY_BOTH);
        assertTextPresent(SUMMARY_ADMIN);
        assertTextNotPresent(SUMMARY_FRED);
